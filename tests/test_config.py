...
def test_load_and_expand_smoke_config() -> None:
    config, digest = load_config(Path("configs/p0a_clean_smoke.yaml"))
    trials = expand_trials(config, digest, Path("configs/p0a_clean_smoke.yaml"))
    assert digest
    assert len(trials) == 6
...
def test_live_panel_config_requires_and_expands_explicit_provider_pins() -> None:
    config, digest = load_config(Path("configs/p0a_clean_live_panel.yaml"))
    trials = expand_trials(config, digest, Path("configs/p0a_clean_live_panel.yaml"))
...
def test_tier_selected_config_expands_only_selected_panel_tier() -> None:
    config, digest = load_config(Path("configs/p0a_clean_live_sweep.yaml"))
    trials = expand_trials(config, digest, Path("configs/p0a_clean_live_sweep.yaml"))
...
def test_dev_mode_allows_openrouter_auto() -> None:
    config, _digest = load_config(Path("configs/openrouter_dev_smoke.yaml"))
